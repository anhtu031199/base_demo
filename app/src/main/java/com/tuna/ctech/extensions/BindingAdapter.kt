package com.tuna.ctech.extensions
...
import com.tuna.ctech.base.adapter.BaseRecyclerAdapter
...
@BindingAdapter("urlAvt", "isCircle")
fun ImageView.loadUrl(urlAvt: String?, isCircle: Boolean?) {
    urlAvt?.let {
        if (isCircle == true) {
            Glide.with(context).load(urlAvt).fitCenter().apply(
                RequestOptions.bitmapTransform(
                    RoundedCornersTransformation(90, 0),
                ),
            ).placeholder(R.drawable.avt_place_holder)
                .into(this)
        } else {
            Glide.with(context).load(urlAvt).fitCenter().placeholder(R.drawable.avt_place_holder)
                .into(this)
        }
    }
}
...
@BindingAdapter("textDepartment")
fun TextView.setDepartment(department: Int) {
    this.text = when (department) {
        1 -> resources.getString(R.string.department_1)
        2 -> resources.getString(R.string.department_2)
        3 -> resources.getString(R.string.department_3)
        4 -> resources.getString(R.string.department_4)
        else -> resources.getString(R.string.department_0)
    }
}
